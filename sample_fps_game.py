...
shoot_sfx = AudioSegment.from_file("C:\\Users\\user\\Documents\\Audacity\\puff.wav")
...
def shoot():
    if not gun.on_cooldown:
        ...
        Thread(target=play, args=(shoot_sfx,)).start()
        invoke(gun.muzzle_flash.disable, delay=.05)
        invoke(setattr, gun, 'on_cooldown', False, delay=.035)
...
class Enemy(Entity):
    def __init__(self, **kwargs):
        super().__init__(parent=shootables_parent, model='cube', scale_y=2, origin_y=-.5, color=color.light_gray, highlight_color=color.orange, collider='box', **kwargs)
        self.health_bar = Entity(parent=self, y=1.2, model='cube', color=color.red, world_scale=(1.5,.1,.1))
        self.max_hp = random.randrange(start=50, stop=500, step=25)
        self.hp = self.max_hp
