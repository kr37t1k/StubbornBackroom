"""
Panda3D Backrooms Game - Complete Implementation with Textures and Audio
This creates a stable 3D Backrooms experience with proper asset loading, textures, and background music
"""
import math
import random
import noise
...
class BackgroundMusic:
    """Background music player for Panda3D"""
    def __init__(self):
        self.music = None
        self.music_manager = base.sfxManagerList[0]  # Use the default audio manager
        
        # Look for audio files in the audio folder
        audio_files = ["atomiste.mp3", "background.mp3", "music.mp3"]
        audio_path = "audio"
        ...
        for filename in audio_files:
            ...
            if os.path.exists(full_path):
                try:
                    self.music = loader.loadSfx(full_path)
                    if self.music:
                        print(f"Loaded background music: {full_path}")
                        break
                except Exception as e:
                    print(f"Could not load {full_path}: {e}")
        
        if not self.music:
            print("Warning: No background music file found in audio folder")
...
class PandaBackrooms(ShowBase):
    def __init__(self):
        ...
        self.background_music.play_audio()
