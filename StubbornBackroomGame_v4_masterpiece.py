#!/usr/bin/env python3
"""
StubbornBackroom: Psycho Dream - Masterpiece Version
Highly optimized with pre-made maps and gymnasium integration
"""
...
class BackroomsGenerator:
    ...
    def _add_room_decorations(self, center, room_type):
        ...
        if room_type in ['liminal', 'chaotic', 'distorted']:
            ...
            if random.random() < 0.05:  # 5% chance of decoration
                self._add_random_decoration(center, room_type)
...
class LiminalParticles(Entity):
    ...
    def __init__(self):
        ...
        self.particles = []
        self.max_particles = 10  # Reduced for better performance
        ...
        for i in range(self.max_particles):
            particle = Entity(
                parent=self,
                model='quad',
                color=color.rgba(255, 245, 220, 50),
                scale=random.uniform(0.03, 0.1),
                position=(
    ...
    def update(self):
        for i, p in enumerate(self.particles):
            ...
            time_factor = 1.0 + reality.distortion_level * 0.2
            p['entity'].y = p['base_pos'].y + math.sin(time.time() * p['speed'] * time_factor + p['offset']) * 0.1
            p['entity'].x = p['base_pos'].x + math.sin(time.time() * 0.1 * time_factor + i + p['offset']) * 0.05
            p['entity'].z = p['base_pos'].z + math.cos(time.time() * 0.15 * time_factor + i + p['offset']) * 0.05
            
            # Simple transparency pulse
            base_alpha = 40 + reality.distortion_level * 20
            pulse = math.sin(time.time() * 0.8 + i + p['offset']) * 10
            alpha = max(10, min(60, base_alpha + pulse))
...
class LiminalMessage:
    ...
    def __init__(self):
        ...
        self.timer = 0
        self.fade_timer = 0
        self.message_interval = 20  # Messages appear every 20 seconds
    ...
    def update(self):
        ...
        if self.timer > self.message_interval and self.fade_timer <= 0:
            ...
            if reality.state == RealityState.CHAOTIC:
                filtered_messages = [msg for msg in messages if "chaos" in msg or "unstable" in msg or "something" in msg]
                if not filtered_messages:
                    filtered_messages = [msg for msg in messages if "reality" in msg]
            elif reality.state == RealityState.LIMINAL:
                filtered_messages = [msg for msg in messages if "liminal" in msg or "between" in msg or "spaces" in msg]
                if not filtered_messages:
                    filtered_messages = [msg for msg in messages if "walls" in msg or "yellow" in msg]
            else:
                filtered_messages = messages
            
            self.current_message = random.choice(filtered_messages) if filtered_messages else random.choice(messages)
            self.fade_timer = 1.0  # Start fading in
            self.timer = 0


class BackroomsGymEnv(gym.Env):
    """
    Gymnasium environment for the Backrooms game
    """
    def __init__(self):
        super(BackroomsGymEnv, self).__init__()
...
window.title = 'Liminalcore Backrooms • Masterpiece Version'
...
print("StubbornBackrooms • Psycho Dream - Masterpiece Version")
print("Controls:")
print("- WASD: Move")
...
if __name__ == "__main__":
    # Example of how to use the gym environment
    print("\nGymnasium environment is available as 'gym_env'")
    print("Example usage:")
    print("  observation, info = gym_env.reset()")
    print("  action = gym_env.action_space.sample()  # Random action")
    print("  observation, reward, terminated, truncated, info = gym_env.step(action)")
