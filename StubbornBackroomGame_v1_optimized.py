#!/usr/bin/env python3
"""
StubbornBackroom: Psycho Dream - Optimized Version 1
No procedural generation - uses pre-made maps only
"""
...
class DebugManager:
    ...
    def update(self):
        ...
        if self.performance_monitor:
            self.perf_text.text = f"Entities: {len(scene.entities)}"
        else:
            self.perf_text.text = ""
        ...
        # Update map info
        if self.map_debug:
            self.map_text.text = f"Map loaded: {world.loaded_map_name}"
        else:
            self.map_text.text = ""
...
class BackroomsGenerator:
    """Optimized backrooms generator using only pre-made maps"""
    def __init__(self):
    ...
    def _create_room_from_map_data(self, x, z, room_type, cell_type):
        ...
        # Create the room using the same method as the original generator
        self.generate_room_at_position(x, z, room_type)
    ...
    def generate_room_at_position(self, x, z, room_type='hallway'):
        """Generate a single room at a specific position"""
    ...
    def _create_walls_with_doorways(self, room_center, room_type, connections):
        ...
        for direction in ['north', 'south', 'east', 'west']:
            if direction not in connections:
                ...
                walls = self._create_wall_with_doorway(room_center, direction, wall_thickness, wall_color, door_size)
...
class LiminalParticles(Entity):
    ...
    def __init__(self):
        ...
        self.particles = []
        self.max_particles = 30  # Reduced for better performance
...
window.title = 'Liminalcore Backrooms • Enhanced Experience (Optimized v1)'
...
print("StubbornBackrooms • Psycho Dream - Optimized Version 1")
print("Controls:")
print("- WASD: Move")
